 ...
 */
async function scheduleEmails(req) {
    const queue = (0, emailQueue_1.getEmailQueue)();
    // Resolve sender
    const senderId = req.senderId ?? (await (0, senderService_1.getDefaultSenderId)());
    if (!senderId) {
    ...
    for (let i = 0; i < req.recipients.length; i++) {
        ...
        const email = await db_1.db.email.create({
            data: {
                senderId,
                recipient,
                subject: req.subject,
        ...
        const jobId = (0, emailQueue_1.makeJobId)(email.id);
        const delayMs = Math.max(0, scheduledTime.getTime() - Date.now());
        const jobData = { emailId: email.id, senderId };
    ...
    console.log(`[Schedule] Scheduled ${req.recipients.length} emails for sender ${senderId}`);
    return {
...
async function reconcilePendingEmails() {
    ...
    const queue = (0, emailQueue_1.getEmailQueue)();
    const pendingEmails = await db_1.db.email.findMany({
    ...
    for (const email of pendingEmails) {
        ...
        requeued++;
        console.log(`[Reconcile] Re-enqueued email ${email.id} (delay: ${Math.ceil(delayMs / 1000)}s)`);
    }
    console.log(`[Reconcile] Done — ${alreadyQueued} already queued, ${requeued} re-enqueued`);
}
